...
import 'package:erp_music/base/buildConfig.dart';
import 'package:erp_music/bean/entity_factory.dart';
import 'package:erp_music/constant/color.dart';
import 'package:erp_music/network/NWApi.dart';
import 'package:erp_music/utils/LogUtil.dart';
...
class HttpManager {
  ...
  static Map<String, CancelToken> _cancelTokens = new Map<String, CancelToken>();
  ...
  PublishSubject<T> get<T>(
    ...
    return _requstHttp<T>(url, true, queryParameters, baseIntercept, isCancelable);
  }

  //post请求
  PublishSubject<T> post<T>(String url,
      {Map<String, dynamic> queryParameters, BaseIntercept baseIntercept, bool isCancelable = true, bool isSHowErrorToast = true}) {
    return _requstHttp<T>(url, false, FormData.fromMap(queryParameters), baseIntercept, isCancelable);
  }
  ...
  PublishSubject<T> _requstHttp<T>(String url, [bool isGet, queryParameters, BaseIntercept baseIntercept, bool isCancelable]) {
    Future future;
    ...
    if (baseIntercept != null) {
      ...
      //为了 能够取消 请求
      if (isCancelable) {
        cancelToken = _cancelTokens[baseIntercept.getClassName()] == null ? CancelToken() : _cancelTokens[baseIntercept.getClassName()];
        _cancelTokens[baseIntercept.getClassName()] = cancelToken;
      }
    }
    if (isGet) {
      future = _dio.get(url, queryParameters: queryParameters, cancelToken: cancelToken);
    } else {
...
//        showToast("登录过期，请重新登录");
      ...
      if (code != 200) {
        ...
        publishSubject.add(EntityFactory.generateOBJ<T>(json.decode(data.toString())));
  ...
  ///请求错误以后 做的一些事情
  void callError(PublishSubject publishSubject, MyError error, BaseIntercept baseIntercept) {
