...
//课程列表Item
class KechengVideoItem extends StatelessWidget {
  KeChengVideo data;

  KechengVideoItem({this.data});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: InkWell(
        onTap: () {
          Navigator.of(context).push(CupertinoPageRoute(builder: (_) => VideoPage2(id: int.parse(data.courseId), videoId: data.id)));
        },
        child: Row(
          children: <Widget>[
            Container(
  ...
  Widget _rightWidget() {
    ...
    for (int i = 0; i < data.videolabel.length; i++) {
